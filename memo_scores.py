...
async def main():
    ...
    PRODUCT,AI,TRADING/PORTFOLIO MANAGEMENT,BUSINESS. In assigning your weights follow these guidelines:
    ...
    Format your response in 1 line without providing further details or calculations:
    
    <USERNAME> : <MODEL WEIGHTED SCORE> 
    ...
    users = [each['user'] for each in json_data if not each['user'] == 'None']
    user_set = list(set(users))
    json_calls = [analyze_json_file(file_path, api_key, prompt, each) for each in user_set]
    
    with open('gpt-4o_wAI_scores1.json', 'w') as f:
        
        #result = await analyze_json_file(file_path, api_key, prompt, user)
        results = await asyncio.gather(*json_calls)
...
#make sure we get memos of ALL transactions
...
#ideas
#1) equity trading know how #2) stock specific knowledge #3) GPUs #4) AI market hardware
#5) gaming?? #6) tech stock #7) nasdaq #8) NVDA options #9) options #10) anomalous options activity etc.


#categories
#1)stock specific
#2)domain specific
#3)tech specific GPUs (how relevant?! indicates understanding or lack of?!)
#4)adjacent domain
#5)biz analysis (how relevant?! indicates understanding or lack of?!) 

#problems: the more details/ fine grained/ higher resolution of a domain/category the less likely
#for any particular user to register - the more information content IF a user has understanding in that
#particular domain
#the broader a category the higher the likelihood many users possess domain knowledge, the less
#significant the knowledge

#INCORPORATE THIS IN YOUR SCORING WEIGHTS AND INCLUDE IN PROMPT FOR AI TO DO THE SAME

#further improvements to be implemented: reduce false negatives & false positives:
#adapt categories/scoring such that user data with less but highly relevant signal 
#still can receive high score and less relevant knowledge gets downrated

#potentially introduce categories with negative weight/score in the future
#are there categories of knowledge that can be reliably classified as showing
#that a user has superficial information that NEGATIVELY impacts his understanding
#/his ability to extract economic value from it akin to the quote
#"If you want to bankrupt a fool, give him information"
